...
- **3rd-place fill is FIFA-exact:** the official 495-combination Annex-C table (`FIFA_THIRDS`, parsed from the Wikipedia template `Template:2026_FIFA_World_Cup_third-place_table`) maps the 8 qualifying groups → each host slot's 3rd; verified to match the broadcast/media brackets (GER+PAR, FRA+SWE, USA+BIH, …). `matchThirds` stays as a constraint-valid fallback.
- **Match grid default + order:** opens on the **newest stage** (latest `M`/matchday), not "Բոլոր փուլերը" — `render()` sets `view.stage` to the last `distinctStages(vm)` once (`_stageDefaulted`); the user can switch freely after. Within a stage the list is **upcoming-first** (`sortUpcomingFirst`: not-yet-played soonest-first on top, completed below most-recent-first).
- **Remaining ideas (next session):** the 3rd-place playoff is parsed (`rounds.third`) but **not drawn** — add a small card if wanted. Only the **projected/group-stage** phase has run against real data; the **live/final** paths (real teams in every slot, live scores on chips, champion flag in the circle) are untested — **re-verify the bracket once the knockouts actually start** (late June 2026).
